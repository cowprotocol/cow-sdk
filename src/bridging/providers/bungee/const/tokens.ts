 ...
 */
export interface BungeeChainConfig {
...
const BUNGEE_CHAIN_CONFIGS: BungeeChainConfig[] = [
  {
...
export const BUNGEE_TOKEN_MAPPING: Partial<Record<TargetChainId, BungeeChainConfig>> = BUNGEE_CHAIN_CONFIGS.reduce(
  (acc, config) => {
    acc[config.chainId] = config
    return acc
  },
  {} as Record<number, BungeeChainConfig>,
)

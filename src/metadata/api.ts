...
import type { ValidationResult, latest } from '@cowprotocol/app-data'
...
export class MetadataApi {
  ...
  async generateAppDataDoc(params?: GenerateAppDataDocParams): Promise<LatestAppDataDocVersion> {
    ...
    const { createReferrerMetadata, createQuoteMetadata, createOrderClassMetadata, createAppDataDoc } = await import(
  ...
  async getAppDataSchema(version: string): Promise<AnyAppDataDocVersion> {
    const { getAppDataSchema } = await import('@cowprotocol/app-data')
  ...
  async validateAppDataDoc(appDataDoc: AnyAppDataDocVersion): Promise<ValidationResult> {
    const { validateAppDataDoc } = await import('@cowprotocol/app-data')

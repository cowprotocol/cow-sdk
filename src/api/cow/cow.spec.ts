...
describe('Cow Api', () => {
  ...
  test('Valid: Get last 5 orders for a given trader ', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.cow.fi/xdai/api/v1/account/0x00000000005ef87f8ca7014309ece7260bbcdaeb/orders?limit=5&offset=0',
      FETCH_RESPONSE_PARAMETERS
    )
  ...
  test('Invalid: Get last 5 orders for an unexisting trader ', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.cow.fi/xdai/api/v1/account/invalidOwner/orders?limit=5&offset=0',
      FETCH_RESPONSE_PARAMETERS
    )
  })
  ...
  test('Valid: Get last 5 orders changing options parameters', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(fetchMock).toHaveBeenCalledWith(
      'https://barn.api.cow.fi/mainnet/api/v1/account/0x00000000005ef87f8ca7014309ece7260bbcdaeb/orders?limit=5&offset=0',
      FETCH_RESPONSE_PARAMETERS
    )
    expect(orders.length).toEqual(5)
  })

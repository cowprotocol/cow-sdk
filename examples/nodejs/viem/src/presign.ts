 ...
 *
 * Here the owner is a local viem account so the example runs end to end. With a Safe, `owner` would
 * be the Safe address and phase 3 would be a Safe transaction proposal instead of
 * `sendTransaction`. Either way the order owner encoded in the UID is the account that must send
 * the transaction.
 *
...
async function main() {
  ...
  setGlobalAdapter(new ViemAdapter({ provider: publicClient, signer: account }))

  const orderSdk = new TradingSdk({ chainId, appCode, signer: account })
  ...
  // Safe proposal service would run. ViemAdapter still needs a provider to construct, but
  // getPreSignCallData only uses it for ABI encoding — no key is available in this environment.
  const signerlessAdapter = new ViemAdapter({ provider: publicClient })
  ...
  const hash = await walletClient.sendTransaction({
    to: preSignCallData.to as `0x${string}`,
    data: preSignCallData.data as `0x${string}`,
    value: BigInt(preSignCallData.value),
  })
  await publicClient.waitForTransactionReceipt({ hash })
  console.log('Order signed on-chain, tx:', hash)
}

 ...
 *
 * The order is signed "externally" — here by a local viem account standing in for
...
async function main() {
  ...
  const coldWallet = privateKeyToAccount(PRIVATE_KEY as `0x${string}`)
  const owner = coldWallet.address
  ...
  setGlobalAdapter(new ViemAdapter({ provider: publicClient }))
  ...
  // Sign the EIP-712 payload the quote came with. Unlike ethers, viem accepts the payload
  // as-is: the `EIP712Domain` entry in `types` is fine (it uses `primaryType` to know what
  // to hash), so there is nothing to strip. The cast only bridges the SDK's loose typed-data
  // types to viem's stricter generic signature; the runtime value is exactly what viem expects.
  const signature = await coldWallet.signTypedData(quoteResults.orderTypedData as unknown as TypedDataDefinition)

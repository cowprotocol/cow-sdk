...
| --------------------------- | ----------------------- | ------------------------- | ----------------- |
| ![Statements](https://img.shields.io/badge/statements-83.51%25-yellow.svg?style=flat) | ![Branches](https://img.shields.io/badge/branches-83.85%25-yellow.svg?style=flat) | ![Functions](https://img.shields.io/badge/functions-63.82%25-red.svg?style=flat) | ![Lines](https://img.shields.io/badge/lines-86.89%25-yellow.svg?style=flat) |
...
- `getOrderToSubmit` - Build the order to submit with an external signature.
...
```typescript
import { setGlobalAdapter, SupportedChainId, OrderKind, TradingSdk } from '@cowprotocol/cow-sdk'
...
- `getOrderToSubmit` defaults to the `EIP712` signing scheme (sign `orderTypedData` via `eth_signTypedData_v4`). To sign with `eth_sign`/`personal_sign` instead, compute the EIP-712 digest of the order and `personal_sign` that, then pass `sdk.getOrderToSubmit({ quoteResults, signingScheme: SigningScheme.ETHSIGN })` — the scheme must match how you signed. `PRESIGN` (on-chain flow) isn't part of this flow, and `EIP1271` (smart-account) support is planned for a later milestone.

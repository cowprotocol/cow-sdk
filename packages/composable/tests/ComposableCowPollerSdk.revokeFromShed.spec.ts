...
describe('ComposableCowPollerSdk revocation from CowShed', () => {
  ...
  test('signs a CowShed factory bundle without submitting across adapters', async () => {
    ...
    for (const adapter of Object.values(adapters)) {
      ...
      const bundle = await params.sdk.signRevokeFromShed({ ...params, ...params.schedule })
      const decoded = decodeFunctionData({ abi: FACTORY_ABI, data: bundle.signedMulticall.data as `0x${string}` })

      expect(bundle.cowShedAccount).toBe(params.cowShedSdk.getCowShedAccount(1, TEST_ADDRESS))
      expect(bundle.cowShedAccount).not.toBe(params.schedule.owner)
        ...
        [
          {
            target: POLLER,
            value: 0n,
            callData: params.sdk.poller.encodeRevokeFromShed(params.schedule),
            allowFailure: false,
      ...
      expect(sign).toHaveBeenCalledWith(
        expect.objectContaining({ chainId: 1, verifyingContract: bundle.cowShedAccount, version: '2.1.0' }),
        expect.any(Object),
  ...
  test('submits the signed factory bundle with its gas limit', async () => {
    ...
    await expect(params.sdk.revokeFromShed({ ...params, ...params.schedule })).resolves.toBe(transaction)
    ...
    expect(args[0][0]?.callData).toBe(params.sdk.poller.encodeRevokeFromShed(params.schedule))
  })
  ...
  test('uses the configured signer and a per-call override for signing and submission', async () => {
    ...
    await sdk.revokeFromShed({ ...params, ...params.schedule })
    const overrideAddress = await override.getAddress()
    await sdk.revokeFromShed({ ...params, ...params.schedule, signer: override, funder: overrideAddress })
  ...
  test('encodes the supplied epoch and schedule before awaiting a wallet', async () => {
    const params = setup()
    const authorization = { ...params, ...params.schedule }
    const original = { ...authorization }
    const pending = params.sdk.signRevokeFromShed(authorization)
    authorization.authEpoch = 8n
    authorization.owner = POLLER
    const result = await pending
    const { args } = decodeFunctionData({ abi: FACTORY_ABI, data: result.signedMulticall.data as `0x${string}` })
    expect(args[0][0]?.callData).toBe(params.sdk.poller.encodeRevokeFromShed(original))
  })
  ...
  test('does not submit when signing or gas estimation fails', async () => {
    ...
    await expect(params.sdk.revokeFromShed({ ...params, ...params.schedule })).rejects.toThrow('Rejected')
    expect(send).not.toHaveBeenCalled()
    sign.mockRestore()

    jest.spyOn(adapters.viemAdapter.signer, 'estimateGas').mockRejectedValue(new Error('Reverted'))
    await expect(params.sdk.revokeFromShed({ ...params, ...params.schedule, gasLimit: undefined })).rejects.toThrow(
  ...
  test('uses the CowShed gas estimate or the caller-provided fallback', async () => {
    ...
    const estimated = await params.sdk.signRevokeFromShed({ ...params, ...params.schedule, gasLimit: undefined })
    expect(estimated.gasLimit).toBe(300_000n)

    estimate.mockRejectedValue(new Error('Reverted'))
    const fallback = await params.sdk.signRevokeFromShed({
      ...params,
      ...params.schedule,
      gasLimit: undefined,

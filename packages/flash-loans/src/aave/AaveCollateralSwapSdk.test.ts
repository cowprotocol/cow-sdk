            ...
            appData: {
              metadata: expect.objectContaining({
                flashloan: expect.objectContaining({
                  ...
                  receiver: AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN],
              ...
              metadata: expect.objectContaining({
                hooks: expect.objectContaining({
                  pre: expect.arrayContaining([
                    expect.objectContaining({
                      target: AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN],
      ...
      test('should generate EIP-1271 signature with correct domain', async () => {
        ...
        expect(signTypedDataSpy).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            version: '1',
            chainId: EvmChains.GNOSIS_CHAIN,
            verifyingContract: AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN],
          }),
          expect.any(Object),
          expect.any(Object),
        )
    ...
    describe('constructor configuration', () => {
      ...
      test('should use default hook adapter addresses when no config provided', async () => {
        ...
        expect(getInstanceCall?.[0]?.args?.[0]).toBe(
          AAVE_HOOK_ADAPTER_PER_TYPE[AaveFlashLoanType.CollateralSwap][EvmChains.GNOSIS_CHAIN],
        )
      })
        ...
        const customConfig = {
          hookAdapterPerType: {
            ...
            [AaveFlashLoanType.CollateralSwap]: {
              ...
              [EvmChains.GNOSIS_CHAIN]: customHookAdapterAddress,
            },
          },
        }
      ...
      test('should use custom hook adapter addresses for different chains', async () => {
        ...
        const customConfig = {
          hookAdapterPerType: {
            ...
            [AaveFlashLoanType.CollateralSwap]: {
              ...AAVE_HOOK_ADAPTER_PER_TYPE[AaveFlashLoanType.CollateralSwap],
              [EvmChains.GNOSIS_CHAIN]: customGnosisAddress,
              [EvmChains.MAINNET]: customMainnetAddress,
            },
          },
        }
      ...
      test('should use default aaveAdapterFactory when not provided', async () => {
        ...
        expect(getInstanceCall).toBeDefined()
        expect(getInstanceCall?.[0]?.address).toBe(AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN])

        // Verify that the default address is used in flashLoanHint
        const swapSettingsCall = (mockPostSwapOrderFromQuote as jest.Mock).mock.calls[0][0]
        const flashLoanHint = swapSettingsCall.appData.metadata.flashloan

        expect(flashLoanHint.receiver).toBe(AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN])
        expect(flashLoanHint.protocolAdapter).toBe(AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN])

        // Verify that the default address is used in pre-hook target
        const preHook = swapSettingsCall.appData.metadata.hooks.pre[0]
        expect(preHook.target).toBe(AAVE_ADAPTER_FACTORY[EvmChains.GNOSIS_CHAIN])
      })
      ...
      test('should use custom aaveAdapterFactory when provided', async () => {
        ...
        const customConfig = {
          aaveAdapterFactory: {
            ...AAVE_ADAPTER_FACTORY,
            [EvmChains.GNOSIS_CHAIN]: customAdapterFactory,
          },
        }
      ...
      test('should use default aavePoolAddress when not provided', async () => {
        ...
        expect(flashLoanHint.liquidityProvider).toBe(AAVE_POOL_ADDRESS[EvmChains.GNOSIS_CHAIN])
      })
      ...
      test('should use custom aavePoolAddress when provided', async () => {
        ...
        const customConfig = {
          aavePoolAddress: {
            ...AAVE_POOL_ADDRESS,
            [EvmChains.GNOSIS_CHAIN]: customPoolAddress,
          },
        }
      ...
      test('should use custom aaveAdapterFactory for different chains', async () => {
        ...
        const customConfig = {
          aaveAdapterFactory: {
            ...AAVE_ADAPTER_FACTORY,
            [EvmChains.GNOSIS_CHAIN]: customGnosisFactory,
            [EvmChains.MAINNET]: customMainnetFactory,
          },
        }
      ...
      test('should use custom aavePoolAddress for different chains', async () => {
        ...
        const customConfig = {
          aavePoolAddress: {
            ...AAVE_POOL_ADDRESS,
            [EvmChains.GNOSIS_CHAIN]: customGnosisPool,
            [EvmChains.MAINNET]: customMainnetPool,
          },
        }

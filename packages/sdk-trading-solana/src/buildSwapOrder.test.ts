...
describe('buildSolanaSwapOrder', () => {
  ...
  it('funds the order rent from the sponsor when one is configured, leaving the owner as authenticator', async () => {
    const solanaQuote = await buildFixtureQuote()
  ...
  it('keeps the order identity independent of who pays, since the sponsor is not part of the intent', async () => {
    const solanaQuote = await buildFixtureQuote()
    const quoteResults = buildFixtureQuoteResults()
